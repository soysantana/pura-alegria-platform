<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pt-4 pb-3 sm:px-6 dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">
                Asignación de Cuidadoras
            </h3>
        </div>

        <div class="flex items-center gap-3">
            <button
                @click="isAssignCaregiverModal = true"
    ...
    <div class="max-w-full overflow-x-auto custom-scrollbar">
        <table class="min-w-full">
            ...
            <!-- table header end -->
            <?php $assignCaregiver = find_all_assigned_caregivers(); ?>
            <!-- table body start -->
            <tbody class="py-3 divide-y divide-gray-100 dark:divide-gray-800">
                <?php foreach ($assignCaregiver as $asgcagr): ?>
                    <tr>
                        <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                            <div class="flex items-center">
                                <div class="flex items-center gap-3">
                                    <div class="h-[50px] w-[50px] overflow-hidden rounded-full">
                                        <img src="/src/uploads/<?php echo remove_junk($asgcagr['caregiver_picture']); ?>" alt="Caregiver">
                                    </div>
                                    <div>
                                        <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                            <?php echo remove_junk(ucfirst($asgcagr['caregiver_name'])); ?>
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                            <div class="flex items-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                    <?php echo remove_junk(ucfirst($asgcagr['infant_name'])); ?>
                                </p>
                            </div>
                        </td>
                        <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                            <div class="flex items-center space-x-2">
                                <?php $deleteActionUrl = '/src/db/caregiver/delete-caregiver.php'; ?>
                                <svg
                                    @click="isDeleteModal = true;
                                    deleteId = '<?php echo $asgcagr['assign_id']; ?>';"
                                ...
                                <svg
                                    @click="isEditCaregiverModal = true;
                                    editCaregiverId = '<?php echo base64_encode((int)$asgcagr['assign_id']); ?>';
                                    editCaregiverName = '<?php echo addslashes(remove_junk($asgcagr['caregiver_id'])); ?>';
                                    editCaregiverInfaname = '<?php echo addslashes(remove_junk($asgcagr['infant_id'])); ?>';
                                    "
                                    class=" cursor-pointer hover:fill-error-500 dark:hover:fill-error-500 fill-gray-700 dark:fill-gray-400"
                                    width="18"
                                    height="18"
