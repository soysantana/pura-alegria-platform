  ...
  <div
    ...
    <div class="px-2 pr-14">
      <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
        Asignar Cuidadora a Infante
      </h4>
      <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
        Completa los datos para registrar la asignación.
      </p>
    </div>
    <form
      class="flex flex-col"
      action="/src/db/caregiver/edit-caregiver.php"
      method="POST">
          ...
          <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
            <div class="col-span-2 lg:col-span-1">
              ...
              <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                <?php $caregivers = find_all_caregiver(); ?>
                <input type="hidden" id="assignId" name="assignId" x-model="editCaregiverId">
                <select
                  id="caregiver"
                  name="caregiver"
                  x-model="editCaregiverName"
                  ...
                  <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" disabled selected hidden>
                    Seleccione una cuidadora
                  </option>
                  <?php foreach ($caregivers as $caregiver): ?>
                    <option value="<?php echo remove_junk($caregiver['id']); ?>" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                      <?php echo remove_junk(ucfirst($caregiver['first_name'])); ?>
                    </option>
                  <?php endforeach; ?>
                </select>
            ...
            <div class="col-span-2 lg:col-span-1">
              ...
              <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                ...
                <select
                  id="infant"
                  name="infant"
                  x-model="editCaregiverInfaname"
                  class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" :class="isOptionSelected &amp;&amp; 'text-gray-800 dark:text-white/90'" @change="isOptionSelected = true">
                  <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" disabled selected hidden>
                    Seleccione un infante
                  </option>
                  <?php foreach ($infants as $infant): ?>
                    <option value="<?php echo remove_junk($infant['id']); ?>" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                      <?php echo remove_junk(ucfirst($infant['first_name'])); ?>
                    </option>
                  <?php endforeach; ?>
                </select>
                <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
