<form
    ...
    <div x-data="billingApp()" class="row rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="border-b border-gray-200 p-4 sm:p-8 dark:border-gray-800">
            <div class="space-y-6">
                <div x-data='{
                        tutors: <?= json_encode(find_all_tutor(), JSON_HEX_APOS | JSON_HEX_QUOT) ?>,
                        infants: <?= json_encode(find_all_infants(), JSON_HEX_APOS | JSON_HEX_QUOT) ?>,
                        filteredInfants: [],
                        selectedTutor: "",
                        selectedInfant: "",
                        tutorName: "",
                        ...
                        filterInfants(tutor_id) {
                            ...
                            this.tutorName = tutor ? tutor.first_name + " " + (tutor.last_name || "") : "";
                            this.filteredInfants = this.infants.filter(i => i.tutor_id == tutor_id);
                            this.selectedInfant = "";
                        },
                        setInfantName() {
                            let infant = this.infants.find(i => i.id == this.selectedInfant);
                            this.infantName = infant ? infant.first_name + " " + (infant.last_name || "") : "";
                        }
                    }'
                    class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div>
                        <input type="hidden" id="receiptNo" value="<?php echo get_receipt_no(); ?>">
                    ...
                    <div class="col-span-full">
                        <div class="grid grid-cols-1 gap-5 sm:grid-cols-4">
                            ...
                            <div>
                                ...
                                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                    <select x-model="serviceType" id="serviceType" name="serviceType" required class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" :class="isOptionSelected &amp;&amp; 'text-gray-800 dark:text-white/90'" @change="isOptionSelected = true">
                                        <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                            ...
                            <div>
                                <label for="dayWeek" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Días a la Semana
                                </label>
                                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                    <select x-model="dayWeek" id="dayWeek" name="dayWeek" required class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" :class="isOptionSelected &amp;&amp; 'text-gray-800 dark:text-white/90'" @change="isOptionSelected = true">
                                        <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                    ...
                    <div class="col-span-full">
                        <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
                            <div>
                                ...
                                <input x-model="startTime" type="text" name="startTime" id="startTime" class="timepicker dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" placeholder="Hora de inicio (desde)">
                            </div>
                            <div>
                                <label for="endTime" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Hora de finalización</label>
                                <input x-model="endTime" type="text" name="endTime" id="endTime" class="timepicker dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" placeholder="Hora final (hasta)">
                            </div>
                            <div>
                                <label for="totalHours" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Total de horas</label>
                                <input x-model="totalHours" type="text" name="totalHours" id="totalHours" readonly class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            </div>
                        </div>
                    </div>
                    <div class="col-span-full">
                        <label for="observations" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                        ...
                        <table class="min-w-full text-left text-sm text-gray-700 dark:border-gray-800">
                            ...
                            <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-white/[0.03]">
                                <template x-for="(product, index) in products" :key="index">
                                    <tr>
                                        <input type="hidden" x-bind:name="`products[${index}][description]`" x-bind:value="product.description">
                                        <input type="hidden" x-bind:name="`products[${index}][quantity]`" x-bind:value="product.quantity">
                                        <input type="hidden" x-bind:name="`products[${index}][price]`" x-bind:value="product.price">
                                        <input type="hidden" x-bind:name="`products[${index}][subtotal]`" x-bind:value="product.subtotal">
                ...
                <div class="mt-5 rounded-xl border border-gray-100 bg-gray-50 p-4 sm:p-6 dark:border-gray-800 dark:bg-gray-900">
                    <div @submit.prevent="addProduct">
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 lg:grid-cols-12">
                            ...
                            <div class="w-full lg:col-span-2">
                                <label for="paymentStatus" class="mb-1 inline-block text-sm font-semibold text-gray-700 dark:text-gray-400">Estado de pago</label>
                                <select x-model="paymentStatus" id="paymentStatus" name="paymentStatus" required class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                                    <option value="" disabled selected>Seleccione un servicio</option>
                            ...
                            <div class="w-full lg:col-span-2">
                                <label for="quantity" class="mb-1 inline-block text-sm font-semibold text-gray-700 dark:text-gray-400">Cantidad</label>
                            ...
                            <div class="w-full lg:col-span-2">
                                <label for="discount" class="mb-1 inline-block text-sm font-semibold text-gray-700 dark:text-gray-400">Descuento</label>
                            ...
                            <div class="flex w-full items-end lg:col-span-2">
                                <button type="button" @click="addProduct" class="hover:bg-brand-600 bg-brand-500 h-11 w-full rounded-lg px-4 py-3 text-sm font-medium text-white transition">
                                    Guardar Producto
                                </button>
                            </div>
                        </div>
