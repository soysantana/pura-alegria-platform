  ...
  <div class="mb-4 flex flex-col gap-2 px-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
    <div>
      <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">
        Listado de los articulos de los infantes
      </h3>
    </div>
    ...
    <div class="flex items-center gap-3">
      ...
      <div class="flex items-center gap-3">
        <div>
          <a href="/item-infant-add" class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium text-white transition">
  ...
  <div class="custom-scrollbar max-w-full overflow-x-auto overflow-y-visible px-5 sm:px-6">
    <table class="min-w-full">
      ...
      if ($user_level == 1 || $user_level == 2) {
        ...
        SELECT 
            i.id AS infant_id,
            CONCAT(i.first_name, ' ', i.last_name) AS infant_name,
            GROUP_CONCAT(ii.name ORDER BY ii.created_at SEPARATOR ', ') AS item_name,
            MAX(ii.created_at) AS last_created,
            MAX(ii.updated_at) AS last_updated
        FROM item_infants ii
        INNER JOIN infants i ON i.id = ii.infant_id
        GROUP BY ii.infant_id
    ...
    ";
        ...
        FROM item_infants ii
        INNER JOIN infants i ON i.id = ii.infant_id
        WHERE ii.tutor_id = '{$user_id}'
        GROUP BY ii.infant_id
        ORDER BY last_updated DESC
    ";
      }

      $item_infants = find_by_sql($sql);
      ?>



      <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
        <?php foreach ($item_infants as $item): ?>
          <tr>
            <td class="py-3">
              <div class="flex items-center">
                <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                  <?= remove_junk(ucfirst($item['infant_name'])); ?>
                </p>
              </div>
            </td>
            <td class="py-3">
              <div class="flex items-center">
                <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">
                  <?= remove_junk(ucfirst($item['item_name'])); ?>
                </p>
              </div>
            </td>
            <td class="py-3">
              <div class="flex items-center">
                <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                  <?= date('d/m/Y h:i A', strtotime($item['last_created'])); ?>
                </p>
              </div>
            </td>
            <td class="py-3">
            ...
            <td class="py-3">
              <div class="flex items-center space-x-2">
                <?php $deleteActionUrl = '/src/db/infant/items/delete-item.php'; ?>
                <svg
                  @click="isDeleteModal = true;
                  deleteId = '<?= (int)$item['infant_id']; ?>'; 
                  "
                ...
                <svg
                  @click="window.location.href = 'item-infant-edit?id=<?php echo base64_encode((int)$item['infant_id']); ?>'"
                  class="cursor-pointer hover:fill-error-500 dark:hover:fill-error-500 fill-gray-700 dark:fill-gray-400"
                  width="18"
                  height="18"
                  viewBox="0 0 18 18"
                ...
                <svg
                  @click="fetch('/src/includes/infant/items/item-view-modal.php', {
                  method: 'POST',
                  headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                  body: 'viewItemId=<?= base64_encode((int)$item['infant_id']); ?>'
                  })
                  .then(r => r.text())
                  .then(html => document.getElementById('itemViewModal').innerHTML = html)
                  "
