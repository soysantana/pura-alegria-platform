                            ...
                            <div class="flex items-center">
                                <div class="flex items-center gap-3">
                                    <div class="h-[50px] w-[50px] overflow-hidden rounded-full">
                                        <img src="/src/uploads/<?php echo remove_junk($infant['picture']); ?>" alt="Infant">
                                    </div>
                                    <div>
                        ...
                        <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                            <div class="flex items-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                    <?php echo remove_junk(ucfirst($infant['last_name'])); ?>
                                </p>
                            </div>
                        </td>
                        <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                            <div class="flex items-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                    <?php echo remove_junk($infant['birth_date']); ?>
                                </p>
                            </div>
                        </td>
                        <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                            <div class="flex items-center space-x-2">
                                <?php $deleteActionUrl = '/src/db/infant/delete-infant.php'; ?>
                                <svg
                                    @click="isDeleteModal = true;
                                    deleteId = '<?php echo (int)$infant['id']; ?>';"
                                ...
                                <svg
                                    @click="isEditInfantModal = true;
                                    editInfantId = '<?php echo (int)$infant['id']; ?>';
                                    editInfantFname = '<?php echo addslashes(remove_junk($infant['first_name'])); ?>';
                                    editInfantLname = '<?php echo addslashes(remove_junk($infant['last_name'])); ?>';
                                    editInfantDate = '<?php echo addslashes(remove_junk($infant['birth_date'])); ?>';
                                    editInfantGender = '<?php echo addslashes(remove_junk($infant['gender'])); ?>';
                                    editInfantTutorId = '<?php echo (int)$infant['tutor_id']; ?>';
                                    "
                                    class=" cursor-pointer hover:fill-error-500 dark:hover:fill-error-500 fill-gray-700 dark:fill-gray-400"
                                    width="18"
                                    height="18"
